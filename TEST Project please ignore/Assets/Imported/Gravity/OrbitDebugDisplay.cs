...
public class OrbitDebugDisplay : MonoBehaviour {
    ...
    void DrawOrbits () {
        ...
        for (int bodyIndex = 0; bodyIndex < virtualBodies.Length; bodyIndex++) {
            var pathColour = bodies[bodyIndex].color;//gameObject.GetComponentInChildren<MeshRenderer> ().sharedMaterial.color; //

            if (useThickLines) {
                var lr = bodies[bodyIndex].gameObject.GetComponentInChildren<LineRenderer>();
                if (!lr) lr = bodies[bodyIndex].gameObject.AddComponent<LineRenderer>();
                var lineRenderer = lr;
